...
description: Esta skill deve ser usada ao prospectar clientes no Google Maps — buscar negócios bem avaliados com sites ruins ou sem site, qualificar leads (tipo redesign ou criação), avaliar qualidade de sites de terceiros e montar a planilha de leads. Acione quando o usuário disser "prospectar", "buscar clientes", "achar leads", "clientes com site ruim", "clientes sem site" ou rodar /prospectar.
---

# Prospecção no Google Maps

Encontrar o cliente ouro: negócio que JÁ fatura bem (nota alta, muitas avaliações) mas perde clientes por causa de um site fraco — ou por não ter site nenhum. Não se cria demanda — conserta-se onde o dinheiro está escapando.
...
2. Percorrer os resultados um a um, em ordem. Para cada estabelecimento:
   ...
   - **Filtro 2 — situação do site (define o `tipo` do lead, nunca descarta por si só)**:
     - Site próprio ativo e acessível → `tipo: redesign` (a oferta é "uma versão muito melhor do SEU site"). Segue pro Filtro 3.
     - Sem site, site fora do ar, ou "site" que é só diretório de terceiros/linktree/rede social sem página própria → `tipo: criacao` (a oferta é "um site profissional para o seu negócio, feito do zero"). Pula direto pra coleta — não há site pra avaliar.
   - **Filtro 3 — site ruim (só para `tipo: redesign`)**: abrir o site em nova aba e avaliar pelos critérios abaixo. Site bom → descartar. Site ativo porém ruim → candidato (falta só o e-mail).
3. Parar ao atingir a meta de leads qualificados (config, padrão 10) ou após avaliar 25 estabelecimentos. Leads `tipo: criacao` contam pra mesma meta.
...
Nome, nota, nº de avaliações, telefone, WhatsApp, e-mail, URL do site (vazio se `tipo: criacao`), motivo, `tipo` (`redesign` ou `criacao`).

**Lead `tipo: criacao` (sem site próprio) — coletar do PERFIL do Google Maps** (substitui a extração que normalmente viria do site): categoria/especialidade, endereço completo, horários de funcionamento, todas as fotos do perfil (aba "Fotos" — role até o fim, colete as URLs das imagens em maior resolução disponíveis), nota + 2-3 avaliações reais em destaque, site de terceiros se houver (Instagram/Linktree — usar só como fonte de contato, nunca como "site atual" na proposta). Esse material alimenta a skill `criacao-premium` no `/criar-site` — sem ele a página nova não pode ser gerada (nada de foto genérica de banco de imagens).
...
```markdown
| # | Nome | Nota | Aval. | E-mail | Telefone | Tipo | Site atual | Motivo | Status | URL nova |
```

Coluna `Tipo`: `redesign` (tem site, vai pro `/redesenhar`) ou `criacao` (sem site, vai pro `/criar-site`). Para `tipo: criacao`, `Site atual` fica vazio e `Motivo` registra "sem site próprio" (+ onde achou contato, ex. "perfil só no Instagram").

Status possíveis: `novo`, `redesenhado`, `publicado`, `proposta enviada` (o mesmo funil serve os dois tipos — para `criacao`, o status `redesenhado` marca "página pronta", preenchido pelo `/criar-site`). Quando um status mudar (redesenhar/criar/publicar/proposta), regenerar a planilha do Google com os dados acumulados e atualizar o `dashboard.html` (skill `dashboard-leads`). Nunca sobrescrever leads antigos — apenas acrescentar e atualizar.
