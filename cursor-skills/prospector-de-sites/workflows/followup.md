...
2. Selecione os elegíveis: leads com status `proposta`, data de envio há **3+ dias**, e SEM follow-up registrado (procure "follow-up" no campo `obs` do banco/`leads.md`). Se `argumentos do usuário` indicar um cliente, restrinja a ele (mas respeite a regra de nunca repetir follow-up).
