...
3. **Salvar** na pasta de trabalho (`prospector-data/`), com o nome do cliente no arquivo:
...
4. **Comparador (OBRIGATÓRIO — não é opcional)**: crie/atualize `comparar.html` na RAIZ da pasta de trabalho (`prospector-data/`) usando `references/comparador-template.html` da skill `redesign-premium`. Para este cliente, use `"old": null` e `"motivo": "Ainda não tinha site — esta é a primeira versão"` no array JSON — o template já mostra o cartão correto sozinho. Se `comparar.html` já existir, LEIA o array atual e acrescente os novos no topo — nunca perca os antigos.
