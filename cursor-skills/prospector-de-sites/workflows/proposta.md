...
2. Determine os destinatários: `argumentos do usuário`, ou todos os leads com status `publicado` que ainda não receberam proposta em NENHUM canal. Para cada um, verifique o que ele tem: e-mail confirmado → entra no lote de e-mail; WhatsApp capturado na prospecção → entra no lote de WhatsApp (a maioria entra nos dois lotes).
...
4. **E-mail** (para quem tem e-mail confirmado): escreva seguindo "E-mail — Estrutura" da skill `proposta-email`, usando os dados reais do lead (elogio das avaliações, defeito/oportunidade específico, link único da capa). NUNCA mencione preço. Rode a "E-mail — Checklist anti-spam" (bloqueante) antes de criar o rascunho. Envio conforme o modo do config: **rascunho** (padrão, via Gmail via browser ou MCP Resend) ou **enviar direto** (MCP `cursor-ide-browser` no Gmail web, se o conector não suportar).
5. **WhatsApp** (para quem tem WhatsApp capturado): escreva seguindo "WhatsApp — Mensagem" da skill `proposta-email` — versão curta e conversacional, NUNCA a mesma redação do e-mail colada. Rode o "WhatsApp — Checklist" (bloqueante). Envio seguindo "WhatsApp — Envio": abra `web.whatsapp.com/send?phone=...&text=...` via MCP `cursor-ide-browser`, confirme que a mensagem carregou no campo de texto e PARE — deixe pronta para o usuário revisar e clicar em enviar. Só clique em enviar você mesmo se o usuário pedir explicitamente e confirmar o lote antes.
