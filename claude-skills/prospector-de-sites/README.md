...
1. `/setup` — roda uma vez: assinatura, nichos padrão, dados FTP da Hostinger (com teste de publicação).
...
5. `/publicar [cliente|todos]` — sobe na Hostinger em `dominio.com/clientes/[slug]/` e verifica a URL.
...
Tudo na pasta conectada: `prospector-config.json` (preferências e credenciais — a senha FTP fica em texto no seu computador, nunca no chat), `leads.md` (pipeline), `sites/[slug]/` (página + editor de cada cliente) e `comparar.html` (comparador antes/depois).
