---
description: Gera follow-up por e-mail e WhatsApp para propostas sem resposta há 3+ dias (1 por lead, nunca repete)
...
2. Selecione os elegíveis: leads com status `proposta`, proposta enviada (e-mail e/ou WhatsApp) há **3+ dias**, e SEM follow-up registrado (procure "Follow-up" no campo `obs` do banco/`leads.md`). Se `$ARGUMENTS` indicar um cliente, restrinja a ele (mas respeite a regra de nunca repetir follow-up).
